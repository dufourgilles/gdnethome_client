...
const genericDatapointPending = state => ({
  ...state,
  isFetching: true,
  error: null
});
...
const genericDatapointSuccess = (state, action) => ({
  ...state,
  isFetching: false,
  items: fixDatapoints(action.datapoints)
});

const genericDatapointFailure = (state, action) => ({
  ...state,
  isFetching: false,
  error: action.error
});
...
export const datapointReducer = createReducer(initialState, {
  ...
  [FETCH_DPTYPES_SUCCESS]: (state, action) => ({
    ...state,
    types: action.dptypes
  })
});
