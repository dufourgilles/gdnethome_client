...
class ActionMonitor extends React.Component {
  state = {
    actionID: "",
    action: null,
    data: null,
  ...
  handleSubmit = () => {
    fetchActionData([...this.state.selectedFiles])
      .then(data => {
        this.setState({ data });
      })
      .catch(e => {
      ...
      return (
        <div key={name}>
          ...
          <input
            ...
            checked={this.state.selectedFiles.has(file)}
          />
        </div>
      );
    });
    return <div className="action-monitor-files-select">{renderedFiles}</div>;
  };
